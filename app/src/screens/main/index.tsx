...
const mapDispatchToProps = dispatch => ({
  ...
  receiveNotify: creds => receiveNotify(creds),
  receiveLogin: creds => receiveLogin(creds)
});
...
class Main extends React.Component<IProps> {
  ...
  constructor(props) {
    ...
    this.socket.on(this.props.user.user._id, data => {
      console.warn(data);
      this.props.receiveNotify(data);
    });
  }

  public componentDidMount() {
    this.updateBooks();
    this.timer = setInterval(() => this.getUser(), 5000);
  }

  async getUser() {
    const res = await fetch(
      "https://epam-lib.herokuapp.com/api/users/" + this.props.user.user._id
    );
